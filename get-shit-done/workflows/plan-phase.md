...
@/Users/rafaelfigueroa/.claude/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node /Users/rafaelfigueroa/.claude/get-shit-done/bin/gsd-tools.cjs init plan-phase "$PHASE")
```
...
```bash
PHASE_INFO=$(node /Users/rafaelfigueroa/.claude/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
```
...
```bash
PHASE_DESC=$(node /Users/rafaelfigueroa/.claude/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
PHASE_REQ_IDS=$(node /Users/rafaelfigueroa/.claude/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section // empty' | grep -i "Requirements:" | head -1 | sed 's/.*Requirements:\*\*\s*//' | sed 's/[\[\]]//g' | tr ',' '\n' | sed 's/^ *//;s/ *$//' | grep -v '^$' | tr '\n' ',' | sed 's/,$//')
```
...
```
Task(
  prompt="First, read /Users/rafaelfigueroa/.claude/agents/gsd-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
```
Task(
  prompt="First, read /Users/rafaelfigueroa/.claude/agents/gsd-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
